...
func NewDaemonSetPodFilter(client kubernetes.Interface) PodFilterFunc {
	return func(p core.Pod) (bool, error) {
		...
		// longer exists.
		if _, err := client.AppsV1().DaemonSets(p.GetNamespace()).Get(context.Background(), c.Name, meta.GetOptions{}); err != nil {
...
func NewStatefulSetPodFilter(client kubernetes.Interface) PodFilterFunc {
	return func(p core.Pod) (bool, error) {
		...
		// longer exists.
		if _, err := client.AppsV1().StatefulSets(p.GetNamespace()).Get(context.Background(), c.Name, meta.GetOptions{}); err != nil {
			if apierrors.IsNotFound(err) {
				return true, nil
			}
